...
class FilesChatAgent:
    ...
    def grade_documents(self, state: GraphState) -> Dict[str, Any]:
        ...
        filtered_docs = []
        for d in documents:
            # Kiểm tra xem d có phải là một chuỗi hay không, nếu là chuỗi thì sử dụng trực tiếp
            document_content = d if isinstance(d, str) else d.page_content
            score = self.document_grader.get_chain().invoke({"question": question, "document": document_content})
